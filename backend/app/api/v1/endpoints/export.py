...
# ── POST endpoint — frontend compatibility ─────────────────────────────────────
...
@router.post("")
async def export_post(req: ExportRequest, db: Session = Depends(get_db)):
    """
    POST /api/v1/export — redirect to appropriate GET handler.
    Accepts { job_id, format } body for frontend compatibility.
    """
    fmt = req.format.lower().strip()
    if fmt in ("excel", "xlsx"):
        return await export_excel(req.job_id, db)
    elif fmt == "csv":
        return await export_csv(req.job_id, db)
    elif fmt == "json":
        return await export_json(req.job_id, db)
    else:
        raise HTTPException(400, f"Unsupported format: {fmt}. Use: excel, csv, json")
