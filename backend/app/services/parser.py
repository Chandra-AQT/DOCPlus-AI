        ...
        for blk in blocks:
            ...
            if btype == 0:  # text block
                ...
                for line in blk.get("lines", []):
                    # Join spans intelligently: detect and fix char-by-char spans
                    spans = line.get("spans", [])
                    if not spans:
                        continue
                    # Check if spans are single characters (spaced font rendering)
                    single_char = sum(1 for s in spans if len(s.get("text","").strip()) <= 1)
                    if len(spans) > 3 and single_char / len(spans) > 0.7:
                        # Concatenate without spaces — these are individual characters
                        spans_text = "".join(s.get("text", "") for s in spans).strip()
                    else:
                        # Normal join: concatenate spans directly (no extra space)
                        spans_text = "".join(s.get("text", "") for s in spans).strip()
                    if spans_text:
...
def _clean_extracted_text(text: str) -> str:
    ...
    1. 'S p a c e d   o u t' characters (each char extracted as separate span)
    2. Excessive whitespace
    3. Lone hyphen continuation lines
    """
    if not text:
        return text
    lines = text.split('\n')
    cleaned = []
    for line in lines:
        # Detect spaced-out characters: single chars separated by spaces
        # Pattern: "U n d e r s t a n d i n g" — every 2nd char is a space
        stripped = line.strip()
        if stripped and len(stripped) >= 4:
            chars = list(stripped)
            # Check if it looks like spaced chars: odd positions are mostly spaces
            spaces_at_odd = sum(1 for i, c in enumerate(chars) if i % 2 == 1 and c == ' ')
            total_odd = max(1, len(chars) // 2)
            if spaces_at_odd / total_odd > 0.7:
                # Remove every other space to collapse
