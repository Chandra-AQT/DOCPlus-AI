...
# Hints used to detect "critical" fields by partial name match.
# These are generic patterns — NOT specific field names.
# A field is considered critical if it looks like a primary identifier.
_CRITICAL_FIELD_HINTS = {
    "number", "id", "code", "name", "model", "part", "sku",
    "ref", "identifier", "serial", "key",
}
...
def compute_quality_for_records(
    ...
    # Use the provided schema_fields as the canonical field list.
    # Fall back to the union of all fields across all records if schema_fields is empty.
    if schema_fields:
        effective_fields = schema_fields
    else:
        effective_fields = list({k for rec in records for k in rec.get("result", {}).keys()})
    ...
    for rec in records:
        ...
        non_null = [f for f in effective_fields if result.get(f) is not None and result.get(f) != ""]
        coverage = len(non_null) / len(effective_fields) if effective_fields else 0

        # Average confidence — use actual values from metadata if available
    ...
    # Count how many fields are filled across all records (for stats display)
    total_filled = sum(
        1 for rec in records for f in effective_fields
        if rec.get("result", {}).get(f) is not None and rec.get("result", {}).get(f) != ""
    )
    total_possible = len(effective_fields) * len(records)
    ...
    return {
        ...
        "stats": {
            ...
            "fill_rate_pct":   round(total_filled / total_possible * 100, 1) if total_possible else 0,
        },
        "missing_critical": [],
        "suggestions": [
            f"{len(records)} records extracted. "
            f"{total_filled}/{total_possible} field values filled ({round(total_filled/total_possible*100) if total_possible else 0}%)."
        ] if records else [],
    }
