...
async def _extract_chunk_multi(
    ...
    # Find the list[object] field to build a more targeted description
    list_field = next(
        (f for f in schema.get("fields", [])
         if f.get("type") in ("list[object]", "list_object") or
         (f.get("type") in ("list", "array") and f.get("fields"))),
        None
    )
    list_field_name = list_field.get("name", "records") if list_field else "records"
    list_field_desc = list_field.get("description", "") if list_field else ""

    array_desc = (
        f"Extract ALL {list_field_name} found in the document as a complete list. "
        f"{list_field_desc} "
        "Read every row from every table. Do not skip any entries. "
        "Extract the exact values as printed — do not round or estimate. "
        "Return null for fields not found for a specific record."
    ).strip()
...
def _ade_extract_response_to_pipeline_result(response_json: dict, schema: dict) -> dict:
    ...
    fields = schema.get("fields", [])
    field_names = [f["name"] for f in fields]
    # Build a type lookup so we can post-process 0 values correctly
    field_type_map = {f["name"]: f.get("type", "string") for f in fields}
    ...
    for fname in field_names:
        ...
        # Post-process: treat 0 as null for numeric/integer fields
        ftype = field_type_map.get(fname, "string")
        if value == 0 and ftype in ("integer", "number", "currency"):
            value = None

        # For nested list[object] fields, post-process each sub-record's numeric fields
        if isinstance(value, list) and value and isinstance(value[0], dict):
            parent_field = next((f for f in fields if f["name"] == fname), {})
            sub_type_map = {sf["name"]: sf.get("type", "string") for sf in parent_field.get("fields", [])}
            cleaned = []
            for rec in value:
                cleaned_rec = {}
                for k, v in rec.items():
                    sf_type = sub_type_map.get(k, "string")
                    if v == 0 and sf_type in ("integer", "number", "currency"):
                        cleaned_rec[k] = None
                    else:
                        cleaned_rec[k] = v
        ...
        # Confidence: use LandingAI metadata confidence if available, otherwise infer
        if isinstance(meta, dict) and "confidence" in meta:
            confidence[fname] = float(meta["confidence"])
        elif value is None:
            confidence[fname] = 0.0
        elif isinstance(value, list) and value:
            # For list[object] fields, compute average fill rate across sub-records
            if isinstance(value[0], dict):
                all_sub_vals = [v for rec in value for v in rec.values()]
                non_null = [v for v in all_sub_vals if v is not None and v != '']
                confidence[fname] = round(len(non_null) / max(1, len(all_sub_vals)) * 0.9, 3)
            else:
                confidence[fname] = 0.9
        else:
            confidence[fname] = 0.9
...
def _ade_multi_response_to_pipeline_result(response_json: dict, schema: dict) -> dict:
    ...
    Each record contains the sub-fields from the list[object] field in the schema.
    Convert each record into a pipeline result with proper confidence scores.
    """
    fields = schema.get("fields", [])
    top_field_names = [f["name"] for f in fields]

    # Find the list[object] field to get its sub-field names
    # These are the actual per-record field names LandingAI returns
    list_obj_field = next(
        (f for f in fields
         if f.get("type") in ("list[object]", "list_object") or
    ...
    if list_obj_field:
        sub_field_names = [sf["name"] for sf in list_obj_field.get("fields", [])]
        record_field_names = sub_field_names if sub_field_names else top_field_names
    else:
        record_field_names = top_field_names
    ...
    # LandingAI metadata for confidence — try multiple locations
    ...
    for i, rec in enumerate(raw_records):
        ...
        # Extract all fields that exist in this record — dynamic, not just pre-defined names
        # This handles cases where LandingAI returns extra or differently-named fields
        all_rec_keys = set(list(rec.keys())) | set(record_field_names)

        for fname in all_rec_keys:
            value = rec.get(fname)
            meta = rec_meta.get(fname, {}) if isinstance(rec_meta, dict) else {}
            refs = meta.get("references", []) if isinstance(meta, dict) else []

            # Get confidence from metadata if available, else use 0.9 for non-null, 0.0 for null
            conf_val = meta.get("confidence", 0.9 if value is not None else 0.0) if isinstance(meta, dict) else (0.9 if value is not None else 0.0)

            result[fname]     = value
            confidence[fname] = float(conf_val) if conf_val is not None else (0.9 if value is not None else 0.0)
            sources[fname]    = "landingai_ade"
            evidence[fname]   = f"LandingAI ADE references: {refs}" if refs else ""
        ...
        records.append({
            ...
            "evidence":     evidence,
            "validation":   {},
            "schema_fields": record_field_names,
        })

    # Deduplicate by primary key
    records = _deduplicate_records(records, record_field_names)

    # Post-process: fix zero values that should be null for numeric fields
    records = _fix_zero_nulls(records, record_field_names)

    # Compute quality score across all records
    from app.services.quality_scorer import compute_quality_for_records
    quality = compute_quality_for_records(records, record_field_names)
...
def _get_primary_key(result: dict, field_names: list) -> str | None:
    """
    Dynamically detect the primary identifier field from schema fields.
    Looks for the first field that is 'required' and has a string type,
    or falls back to the first non-null string value.
    No hardcoded field names — works with any schema.
    """
    # Try to find a field marked as primary_identifier or required string-type
    # by checking common identifier patterns in the field name
    id_hints = ("number", "id", "code", "num", "no", "sku", "ref", "key", "identifier")
    for fname in field_names:
        val = result.get(fname)
        if val is not None and isinstance(val, str) and val.strip():
            fname_lower = fname.lower()
            if any(hint in fname_lower for hint in id_hints):
                return val.strip()
    # Fall back to the first non-null value
    for fname in field_names:
        val = result.get(fname)
...
def _deduplicate_records(records: list, field_names: list) -> list:
    """
    Merge duplicate records based on a dynamically detected primary key.
    Schema-driven — no hardcoded field names.
    When two records share the same key:
    ...
    """
    seen: dict = {}  # primary_key -> index in deduped list
    deduped: list = []

    for rec in records:
        result = rec.get("result", {})
        model = _get_primary_key(result, field_names)
...
def _fix_zero_nulls(records: list, field_names: list) -> list:
    """
    For numeric fields: replace 0 with null when the field type suggests
    it's a measurement/quantity that can't realistically be zero.
    Schema-driven — detects numeric fields by type, not by hardcoded name.
    """
    # We can't check schema field types here (only names available),
    # so we use a simple heuristic: if a numeric field value is exactly 0
    # AND the field name contains weight/dimension/capacity keywords,
    # treat it as null. This replaces the old hardcoded whitelist.
    zero_hint_patterns = (
        "weight", "width", "height", "length", "depth", "diameter",
        "capacity", "output", "input", "amps", "mca", "mop", "kw", "mbh",
        "btu", "watt", "volt", "amp", "ton", "cfm", "gpm", "psi",
    )
    for rec in records:
        result = rec.get("result", {})
        conf = rec.get("confidence", {})
        for fname in field_names:
            if result.get(fname) == 0:
                fname_lower = fname.lower()
                if any(p in fname_lower for p in zero_hint_patterns):
                    result[fname] = None
                    conf[fname] = 0.0
    return records
...
def _schema_to_ade_format(schema: dict) -> dict:
    ...
    # These are keyed by field name patterns, not hardcoded exact names.
    diagram_field_hints = {}

    def _get_desc(field: dict) -> str:
        """Get the best description for a field, checking diagram hints first."""
        fname = field["name"]
        # Use stored description — it already contains detailed extraction instructions
        return field.get("description") or field.get("instruction") or fname.replace("_", " ")

    properties: dict = {}
    for field in schema.get("fields", []):
        fname = field["name"]
        ftype = field.get("type", "string")
        desc = _get_desc(field)

        # list[object] and list with sub-fields → JSON Schema array of objects
        if ftype in ("list[object]", "list_object") or (ftype in ("list", "array") and field.get("fields")):
            sub_fields = field.get("fields", [])
            if sub_fields:
                sub_props = {}
                for sf in sub_fields:
                    sfname = sf["name"]
                    sf_desc = _get_desc(sf)
                    sub_props[sfname] = {
                        "type": ["null", type_map.get(sf.get("type", "string"), "string")],
                        "description": sf_desc
                    }
                properties[fname] = {
                    "type": "array",
            ...
            # Plain list with no sub-fields — array of strings
            properties[fname] = {"type": "array", "items": {"type": "string"}, "description": desc}
        elif ftype == "object":
            sub_fields = field.get("fields", [])
            sub_props = {}
            ...
            # Scalar field — allow null so LandingAI doesn't raise schema violations
