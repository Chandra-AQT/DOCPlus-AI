...
class ExtractionJob(Base):
    ...
    user_id     = Column(String, nullable=True, index=True)   # admin user owner
    guest_id    = Column(String, nullable=True, index=True)   # guest owner (persists forever)
    document_id = Column(String, nullable=False)
    schema_name = Column(String, nullable=True)
    schema_id   = Column(String, nullable=True)
    batch_id    = Column(String, nullable=True)
    batch_run_id = Column(String, nullable=True, index=True)  # groups all jobs from one extraction run
    ...
    credits_used     = Column(Float, nullable=True, default=0)  # LandingAI credits consumed
